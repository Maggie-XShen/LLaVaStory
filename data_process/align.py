...
image_path = '/content/StorySalon/Image_inpainted/Video'
txt_path = '/content/rawText'
...
def align_dtw(sub_dir):
    ...
    try:
        txt_file_path = os.path.join(txt_path, sub_dir, sub_dir + '.txt')
        for line in open(txt_file_path).readlines():
            if line.strip():
                ListFromLine = line.strip().split('->')
                if ListFromLine[1] not in ['[Applause]', '[Music]', '[Music][Applause]', '[Applause][Music]']:
                    text_timestamp.append(ListFromLine[0].replace(':', '-').replace('.', '-'))
                    text.append(ListFromLine[1])
                    whole_text += ' ' + ListFromLine[1]
    except Exception as e:
        print(f"File failure occurs: {e}")
        return

    try:
        whole_text = Punctmodel.restore_punctuation(whole_text)
    except Exception as e:
        print(f"Punctuation failure occurs: {e}")
        return
    ...
    for sentence in sentences:
        ...
        tmp_sentence = sentence.translate(str.maketrans("", "", string.punctuation + " ")).lower()
        flag = 0
        for i in range(l):
            if i < tmp_i:
                continue
            text_cleaned = text[i].strip().translate(str.maketrans("", "", string.punctuation + " ")).lower()
            if text_cleaned in tmp_sentence or tmp_sentence in text_cleaned or (i > 0 and tmp_sentence in (text[i-1] + text[i]).strip().translate(str.maketrans("", "", string.punctuation + " ")).lower()):
    ...
    images_path = os.path.join(image_path, sub_dir)
    ...
    for image_name in sorted(os.listdir(images_path), key=fns):
        ...
        if os.path.splitext(os.path.basename(i_path))[1] == '.jpg':
            ...
            image_timestamp = os.path.splitext(os.path.basename(i_path))[0].split("_")[-1]
    ...
    try:
        ...
        # Write to txt file
        path = list(zip(p, q))
        txt_name = os.path.join(txt_path, sub_dir, sub_dir + ".txt")
        ...
        print(f"FastDTW failure occurs: {e}")
...
if __name__ == '__main__':
    ...
    for i, sub_dir in enumerate(folders_dir):
        try:
            ...
            print(f"Failure occurs at {sub_dir}: {e}")
            continue
